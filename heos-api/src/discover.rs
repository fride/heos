...
pub async fn find_heos_devices() -> HeosResult<IpAddr> {
    ...
    while let Some(device) = responses.try_next().await.context("Failed to query for upnp devices")? {
        // wow, so much parser nonsense!
        match device.search_target() {
            SearchTarget::URN(urn) if urn.domain_name() == "schemas-denon-com" && urn.typ() == "ACT-Denon" => {
                info!("Found a heos device");
                let url = Url::parse(device.location()).context("UPNP URL not parseable")?;
                let host = url.host().ok_or(anyhow!("Url without host"))?;
                let ip = IpAddr::from_str(&host.to_string()).with_context(|| "Failed to parse ip address")?;
                return Ok(ip);
            },
            _ => {
