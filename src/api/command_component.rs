...
pub async fn heos_api_component(
    ...
    let _ = connection
        .write_frame("system/register_for_change_events?enable=off")
        .await?;
    ...
    tokio::spawn(async move {
        while let Some(cmd) = command_receive.recv().await {
            let res = match cmd {
                ...
                ApiCommand::GetNowPlaying(pid) => {
                    get_now_playing(pid, &mut connection, &results).await
                }
...
async fn get_groups(
    ...
    result_sender
        .send(PlayerUpdate::Groups(response))
        .await
        .unwrap();
    Ok(())
}
...
async fn get_now_playing(
    ...
    let response: PlayerNowPlayingMedia = send_command(
        connection,
        &format!("player/get_now_playing_media?pid={}", player_id),
    )
    .await?;
    result_sender
        .send(PlayerUpdate::NowPlaying(response))
        .await
        .unwrap();
    Ok(())
}

async fn get_music_sources(
    connection: &mut Connection,
    result_sender: &Sender<PlayerUpdate>,
) -> HeosResult<()> {
    let response: Vec<MusicSource> =
        send_command(connection, &format!("browse/get_music_sources")).await?;
    result_sender
        .send(PlayerUpdate::MusicSources(response))
        .await
        .unwrap();
    Ok(())
}
