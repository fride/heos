    ...
    pub async fn execute_command(&mut self, command: HeosCommand) {
        match command {
            HeosCommand::GetPlayer(player_id, responder) => {
                let response = self
                    .connection
                    .execute_command(format!("player/get_player?pid={}", player_id))
                    .await;
...
async fn create_event_channel(mut connection: Connection) -> HeosResult<EventChannel> {
    ...
    let _: RegisteredForChangeEvents = connection
        .execute_command("system/register_for_change_events?enable=on")
        .await?;
    let _ = tokio::spawn(async move {
        loop {
            let event = connection.read_event().await.and_then(|e| e.try_into());
