...
pub trait ExecutableHeosCommand {
    ...
    async fn parse_payload(&self, connection: &mut Connection) -> HeosResult<Self::CommandResultType>;
}

#[async_trait]
impl<R> ExecutableHeosCommand for HeosCommand<R>
    where R: TryFrom<CommandResponse, Error=HeosError> + Send + std::marker::Sync {
...
pub async fn handle_command<R>(connection: &mut Connection,
...
pub mod different_structs {
    ...
    pub trait HeosCommandExecutor {
        async fn execute<C, R>(&mut self, command: C) -> HeosResult<R>
            where C: HeosCommand<CommandResultType=R> + Send + std::marker::Sync,
    ...
    pub fn create_command_handler(mut connection: Connection) -> (mpsc::Sender<HeosCommands>,
        ...
        tokio::spawn(async move {
            while let Some(cmd) = r.recv().await {
                ...
                let result = match cmd {
                    HeosCommands::GetPlayers(get_players) => {
                        connection.execute(get_players)
                            .await
                            .map(|r| HeosCommandResult::PlayerInfos(r))
                    }
                    HeosCommands::GetPlayerVolume(get_player_volume) => {
                        connection.execute(get_player_volume)
                            .await
                            .map(|r| HeosCommandResult::PlayerVolume(r))
                    }
                    HeosCommands::GetGroups(get_groups) => {
                        connection.execute(get_groups)
                            .await
                            .map(|r| HeosCommandResult::Groups(r))
                    }
                    HeosCommands::GetGroupVolume(get_group_volume) => {
                        connection.execute(get_group_volume)
                            .await
