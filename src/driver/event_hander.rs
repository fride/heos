...
fn event_to_results(event: HeosResult<HeosEvent>) -> (Vec<ApiCommand>, Vec<ApiResults>) {
    match event {
        Err(err) => {
            (vec![], vec![ApiResults::Error(err)])
        }
        Ok(HeosEvent::PlayersChanged) => {
            (vec![ApiCommand::GetPlayers], vec![])
        }
        Ok(HeosEvent::SourcesChanged) => {
            (vec![], vec![])
        }
        Ok(HeosEvent::GroupChanged) => {
            (vec![ApiCommand::GetGroups], vec![])
        }
        Ok(HeosEvent::PlayerStateChanged { player_id, state }) => {
            (vec![], vec![ApiResults::PlayerPlayStateChanged(player_id, state)])
        }
        ...
        Ok(HeosEvent::PlayerPlaybackError { player_id, error }) => {
            (vec![], vec![])
        }
        Ok(HeosEvent::PlayerVolumeChanged {
            ...
            (vec![], vec![ApiResults::GroupVolumeChanged(player_id, level, mute)])
        }
        Ok(HeosEvent::PlayerQueueChanged { .. }) => {
            (vec![], vec![])
        }
        Ok(HeosEvent::PlayerRepeatModeChanged { player_id, repeat }) => {
            (vec![], vec![ApiResults::PlayerRepeatModeChanged(player_id, repeat)])
        }
        ...
        Ok(HeosEvent::GroupVolumeChanged {
            ...
            (vec![], vec![ApiResults::GroupVolumeChanged(group_id, level, mute)])
        }
    ...
    tokio::spawn(async move {
        ...
        while let Some(event) = events.next().await {
            ...
            for command_to_send in commands_to_send {
                commands.send(command_to_send).await;
            }
            for result_to_send in results_to_send {
                results.send(result_to_send).await;
            }
        }
    });
}
